...
export default function QuickViewModal({ product, onClose }) {
 ...
 const discountPrice = calcSalePrice(product?.price ?? 0, product?.discount ?? 0);
 const isSaved = isInWishlist(productId);
 const isOutOfStock = (product?.stock ?? 0) <= 0;
 const isLowStock = !isOutOfStock && (product?.stock ?? 99) > 0 && (product?.stock ?? 99) <= 5;
 ...
 return (
 <AnimatePresence>
 {product && <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
 <motion.div
 initial={{ opacity: 0 }}
 animate={{ opacity: 1 }}
 exit={{ opacity: 0 }}
 ...
 <span className="text-2xl font-black text-stone-900 font-mono">{formatPKR(discountPrice)}</span>
 {product.discount > 0 && (
 ...
 {[product.color].filter(Boolean).map((clr) => (
 <button
 key={clr}
